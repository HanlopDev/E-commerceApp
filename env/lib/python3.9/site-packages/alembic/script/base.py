...
class ScriptDirectory:
    ...
    def from_config(cls, config: Config) -> ScriptDirectory:
        ...
        if version_locations_str:
            version_path_separator = config.get_main_option("version_path_separator")
            ...
            try:
                split_char: Optional[str] = split_on_path[version_path_separator]
    ...
    def get_all_current(self, id_: Tuple[str, ...]) -> Set[Optional[Script]]:
        with self._catch_revision_errors():
            return cast(Set[Optional[Script]], self.revision_map._get_all_current(id_))
    ...
    def _stamp_revs(self, revision: _RevIdType, heads: _RevIdType) -> List["StampStep"]:
        with self._catch_revision_errors(
    ...
    def _generate_create_date(self) -> "datetime.datetime":
        if self.timezone is not None:
            if tz is None:
                raise util.CommandError(
                    "The library 'python-dateutil' is required " "for timezone support"
                )
