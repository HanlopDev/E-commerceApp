...
class CycleDetected(RevisionError):
    ...
    def __init__(self, revisions: Sequence[str]) -> None:
        ...
        super(CycleDetected, self).__init__(
            "%s is detected in revisions (%s)" % (self.kind, ", ".join(revisions))
        )
...
class RevisionMap:
    ...
    def get_current_head(self, branch_label: Optional[str] = None) -> Optional[str]:
            ...
            if len(resolved_id) == 1:
                try:
                    ...
                    if rint < 0:
                        ...
                        return tuple(
                            self._walk(head, steps=rint) for head in select_heads
                        )
                except ValueError:
            ...
            for check_head_index, ancestors in enumerate(ancestors_by_idx):
                ...
                if not heads_to_add:
                    ...
                    if (
                        ...
                        ancestors_by_idx[current_candidate_idx] = get_ancestors(
                            heads_to_add[0]
                        )
            ...
            if steps > 0:
                ...
                if initial == "base":
                    ...
                    children = self.get_revisions(
                        self.heads if initial is None else initial.down_revision
                    )
                    if not children:
                        children = ("base",)
            if not children:
    ...
    def _collect_downgrade_revisions(
        ...
        if target_revision is None:
            ...
            roots = [
                is_revision(rev) for rev in self.get_revisions(target_revision.nextrev)
            ]
    ...
    def _collect_upgrade_revisions(
        ...
        required_node_set = set(
            self._get_ancestor_nodes(targets, check=True, include_dependencies=True)
        ).union(targets)

        current_revisions = self.get_revisions(lower)
        if not implicit_base and any(
            rev not in required_node_set for rev in current_revisions if rev is not None
        ):
